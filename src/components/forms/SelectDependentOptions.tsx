                    ...
                    {fields.map((field, index) => (
                        <>
                            <div key={field.id} className="grid grid-cols-7 gap-3 my-2">
                                <Input className="col-span-2" {...register(`dependentOptions.${optionGroup}.${index}.value`, { required: true })} placeholder="Valor" />
                                <Input className="col-span-4" {...register(`dependentOptions.${optionGroup}.${index}.description`, { required: true })} placeholder="Descripción" />
                                <div className='flex flex-row gap-2'>
                                    <Button variant='destructive' title='Eliminar opción' onClick={() => remove(index)}><Trash2 /></Button>
                                    {
                                        index === fields.length - 1 &&
                                        <Button type="button" variant="outline" title='Nueva opción' onClick={() => append({ description: "", value: "" })}>
                                            <Plus />
                                        </Button>
                                    }
                                </div>
