...
const EditQuestion = ({ setOpen, questionId }: EditQuestionProps) => {
    ...
    const editPregunta = ({ questionEdited, questionId }: { questionEdited: PreguntaWithSectionId, questionId: string }) => {
        ...
        const originalSection = data.find(section =>
            section.preguntas.some(q => q.id === questionId)
        )
        const originalSectionId = originalSection?.id

        const sectionWithQuestionEdited = data.map(section => {
            if (section.id === originalSectionId) {
                return {
                    ...section,
                    preguntas: section.preguntas.filter(q => q.id !== questionEdited.id)
                }
            }
            if (section.id === questionEdited.sectionId) {
                return {
                    ...section,
                    preguntas: [...section.preguntas, questionEdited]
                }
            }
            return section
        })
