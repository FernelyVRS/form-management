...
import type { PreguntaWithSectionId } from '@/types';
...
const QuestionForm = ({ setOpen, onSubmit, isChild, isLoading }: QuestionFormProps) => {
    const formStructure = useFormStructureStore(store => store.formStructure)
    ...
    const questionTypeOptions = useMemo(() =>
        questionTypes.map(x => ({ value: x, label: x })),
        []
    )

    const handlerFormSubmit = useCallback((e: React.FormEvent) => {
        if (isChild) {
            e.stopPropagation()
        }
        handleSubmit(onSubmit)(e)
    }, [isChild, handleSubmit, onSubmit])
        ...
        <form onSubmit={handlerFormSubmit}>
            <div className="grid gap-4 py-4">
                ...
                {questionType === 'selectDependent' && (
                    <SelectDependedTypeFields questionsSameSection={formStructure} />
                )}
                {questionType === 'composed' && (
                    <ComposedTypeFields />
                )}
